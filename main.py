...
parser.add_argument('--nin', type=int, default=1)

parser.add_argument('--tr', type=float, default=0.6)
parser.add_argument(
    '--path', type=str, default='./datasets_3/')
...
def train(epoch, X, A, T, Y1, Y0, idx_train, idx_val, model, optimizer):
    ...
    if args.normy:
        ...
        YFtr = YF[idx_train]
        YFva = YF[idx_val]
    #print("yf_pred and YF",yf_pred.shape,YF.shape)
    #print("p1 and T",p1.shape,T.shape)
...
if __name__ == '__main__':
    ...
    for i_exp in range(10):
        ...
        t_total = time.time()
        #print(model)
        last_att = model.a#.cpu().detach().numpy()
        #for para in model.parameters():
            #print("model parameter:")
            #print(type(para.data),para.size())
        ...
        #np.savetxt('att.txt',curr_att.cpu().detach().numpy().max(1))
        print("Optimization Finished!")
        print("Total time elapsed: {:.4f}s".format(time.time() - t_total))

        # Testing
        result  = eva(X, A, T, Y1, Y0, idx_train, idx_test, model, i_exp)
        record.append(result)
    #print(record)
    result_mean = np.array(record).mean(0)
    print(result_mean)
